...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        for i, layer in enumerate(self.layers):
            ...
            x = layer(x, mask, i)
            if i == 0:
                atten_scores = layer.self_attn.attn
...
class Transformer(nn.Module):
    def __init__(self, n_layers=3, d_model=256, d_ff=512, h=8, length=27, stride_num=None, dropout=0.1):
        ...
        self.model = self.make_model(N=n_layers, d_model=d_model, d_ff=d_ff, h=h, dropout=dropout, length=self.length,
                                     stride_num=self.stride_num)

    def forward(self, x, mask=None):
        if self.model is not None:
            x, atten_scores = self.model(x, mask)

        return x, atten_scores

    def make_model(self, N=3, d_model=256, d_ff=512, h=8, dropout=0.1, length=27, stride_num=None):
        if N > 0:
            c = copy.deepcopy
            ...
            model_EncoderLayer = []
            for i in range(N):
                ff = PositionwiseFeedForward(d_model, d_ff, dropout, i, stride_num)
                model_EncoderLayer.append(EncoderLayer(d_model, c(attn), c(ff), dropout, stride_num, i))
